from django.conf import settings
from django.contrib.auth.models import AbstractUser
from django.core.mail import send_mail
from django.db import models
from django.urls import reverse
from django.utils.timezone import now


class User(AbstractUser):
    """Расширенная модель пользователя"""
    image = models.ImageField(upload_to='users_images', null=True, blank=True)
    is_verified_email = models.BooleanField(default=False)


class EmailVerification(models.Model):
    """Модель верификации пользователя"""
    code = models.UUIDField(unique=True)
    user = models.ForeignKey(to=User, on_delete=models.CASCADE)
    timestamp = models.DateTimeField(auto_now_add=True)
    expiration = models.DateTimeField()

    class Meta:
        verbose_name = 'Верификация'
        verbose_name_plural = 'Верификации'
        ordering = ['timestamp']

    def __str__(self):
        return f'Верификация для {self.user.email}'

    def send_verification_email(self) -> None:
        """Отправка письма для подтверждения почты"""
        link = reverse('users:verification', kwargs={
            'email': self.user.email,
            'code': self.code
        })
        verification_link = f'{settings.DOMAIN_NAME}{link}'
        subject = f'Подтверждение аккаунта для {self.user.username}'
        message = f'Для подтверждения аккаунта {self.user.email} перейдите по ссылке: {verification_link}'
        send_mail(
            subject=subject,
            message=message,
            from_email=settings.EMAIL_HOST_USER,
            recipient_list=[self.user.email],
            fail_silently=False,
        )

    def is_expired(self):
        """Проверка срока действия кода подтверждения"""
        return True if now() >= self.expiration else False
